...
// marine snow, the depth gauge reading this page's chapters, the sound
// switch, water that darkens with depth, and the sea-glass header. Pages
// only bring their chapters and their content.

const ReadyContext = createContext(true);
export const useDescentReady = () => useContext(ReadyContext);

interface ShellProps {
  chapters: Chapter[];
  children: ReactNode;
  /** Play the opening sheet (home only). */
  opening?: boolean;
  snow?: number;
  footer?: boolean;
  chat?: boolean;
...
export function DescentShell({ chapters, children, opening = false, snow = 80, footer = true, chat = true, className = "" }: ShellProps) {
  ...
  return (
    <ReadyContext.Provider value={ready}>
      <motion.div className={`sea relative min-h-screen ${className}`} style={{ backgroundColor: water }} data-testid="descent">
        {opening && <Opening onDone={() => setReady(true)} />}
        <Reticle />
